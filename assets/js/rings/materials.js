...
export function createMaterials(tier, gemEnv) {
  const metals = {
    ...
    // reflectance so it does not read as grey next to the golds. 0.13, down
    // from 0.15: at hero scale the shank is a quarter of the frame, and the
    // extra crispness in the highlight reads as polish rather than glass.
    platinum: metal([0.76, 0.74, 0.72], 0.13),
  ...
  // Lifted from 0.62 for the landing film: the hero holds one stone at many
  // times the size the four-ring reel showed it, and at that magnification
  // the fire carries the whole reading. Still a suggestion, never a decal.
  const fire = tier === "low" ? 0.3 : 0.72;
  ...
  const gemInside = new MeshPhysicalMaterial({
    ...
    color: linear(0.9, 0.94, 1.0),
    metalness: 1,
    // Sharper than the old stage's 0.015: the pavilion mosaic is the middle
    // of the stone, and at hero scale a blurred mip smears its creases.
    roughness: 0.01,
    side: BackSide,
    envMap: gemEnv,
  ...
  const gemSurface = new MeshPhysicalMaterial({
    ...
    side: FrontSide,
    envMap: gemEnv,
    // Down from the reel's 1.5: the crown pass is additive, and over a stone
    // this large it was reading as a veil rather than as flashes. The crown
    // still clips to white on the facets that find a card; it no longer
    // frosts the ones that don't.
    envMapIntensity: 1.25,
    // Well under the inside pass's. Fire on the crown tints whole kites —
    // at hero scale that read as a lilac glaze over the stone — while fire
    // in the pavilion shows as sparks between the creases, which is where a
    // real stone actually carries its colour.
    iridescence: fire * 0.35,
    iridescenceIOR: 2.1,
